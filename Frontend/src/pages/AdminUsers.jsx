...
const UserManagement = () => {
  ...
  const fetchMembers = async () => {
    try {
      const res = await axios.get('http://localhost:5176/api/AdminDashboard/members', {
  ...
  const fetchStaff = async () => {
    try {
      const res = await axios.get('http://localhost:5176/api/AdminDashboard/staffs', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setStaff(res.data);
    } catch (err) {
  ...
  const handleAddStaff = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://localhost:5176/api/AdminDashboard/staffs', newStaff, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setSuccessMessage(`Successfully added ${newStaff.name} as a staff member`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://localhost:5176/api/AdminDashboard/staffs/${staffToDelete.staffId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Show success notification
      setSuccessMessage(`Successfully removed ${staffToDelete.email} from staff members`);
      setShowSuccessNotification(true);
  ...
  const handleDeleteStaff = async (staffId) => {
    ...
    try {
      await axios.delete(`http://localhost:5176/api/AdminDashboard/staffs/${staffId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmDeleteMember = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5176/api/AdminDashboard/members/${memberToDelete.memberId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      >
        ...
        >
          <div className="flex flex-col">
            ...
            <span className="text-emerald-100 text-sm mt-0.5">{successMessage}</span>
          </div>
        </Alert>
      </Snackbar>
